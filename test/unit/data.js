...
describe('data API', function() {
  ...
  describe('Create document', function() {
    ...
    it('Call the proper route', async function() {
      ...
      mock
        .lastUrl('CreateDoc')
        .should.equal('http://my.cozy.io/data/io.cozy.testobject/')
      mock
        .lastOptions('CreateDoc')
        .should.have.property('body', '{"test":"value"}')
  ...
  describe('Fetch document', function() {
    ...
    it('Call the proper route', async function() {
      ...
      mock
        .lastUrl('GetDoc')
        .should.equal('http://my.cozy.io/data/io.cozy.testobject/42')
  ...
  describe('Fetch multiple documents at once', function() {
    ...
    it('Call the proper route', async function() {
      ...
      mock
        .lastUrl('GetManyDocs')
        .should.equal(
          'http://my.cozy.io/data/io.cozy.testobject/_all_docs?include_docs=true'
        )
      mock
        .lastOptions('GetManyDocs')
        .should.have.property('body', '{"keys":["42","43"]}')
    ...
    it('Fails when ids is not an array', async function() {
      for (const ids of [undefined, null, 'foo', { foo: 'bar' }]) {
        await should(
          cozy.client.data.findMany('io.cozy.testobject', ids)
        ).be.rejectedWith(/ids/)
      }
    })
  })
  ...
  describe('Fetch the changes feed', function() {
    ...
    it('Call the proper route', async function() {
      let fetched = await cozy.client.data.changesFeed('io.cozy.testobject', {
        since: 0
      })
      ...
      mock
        .lastUrl('ChangesFeed')
        .should.equal(
          'http://my.cozy.io/data/io.cozy.testobject/_changes?since=0'
        )
  ...
  describe('Delete document', function() {
    ...
    it('Call the proper route', async function() {
      ...
      mock
        .lastUrl('DeleteDoc')
        .should.equal(
          'http://my.cozy.io/data/io.cozy.testobject/42?rev=1-5444878785445'
        )
